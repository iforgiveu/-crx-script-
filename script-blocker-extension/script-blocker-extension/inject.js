...
(function () {
    ...
    (function () {
        ...
        let containsKeyword = new originalFunction(`
        const keywords = ${JSON.stringify(keywordsToBlock)};
        return function containsKeyword(str) {
    ...
    `)();
        const scriptElementPool = new WeakSet();
        function defineScriptProperties(element) {
            if (scriptElementPool.has(element))
                return;
            scriptElementPool.add(element);
            let blocked = false;
            const internal = {};
            internal.src = element.src;
            internal.innerHTML = element.innerHTML;
            internal.textContent = element.textContent;
            internal.innerText = element.innerText;
            const properties = {
                src: {
                default:
                    ''
                    ...
                    set: function (value) {
                        if (!blocked && typeof value === 'string') {
                            if (containsKeyword(value)) {
                                logBlock('createElement.' + prop, 'matched', value, '动态脚本', element);
                                blocked = true;
                                internal[prop] = properties[prop].default;
                                return;
                            }
                        }
                        internal[prop] = value;
        ...
        document.createElement = function (tagName, options) {
            ...
            if (tagName === 'script') {
                defineScriptProperties(element);
            }
            return element;
        };
    })();
                ...
                if (node.nodeName === 'SCRIPT' && node.type === 'module') {
                    let src = getSafeSrc(node)
                        if (src) {
                            for (const kw of keywordsToBlock) {
                                if (src.includes(kw)) {
                                    logBlock('module脚本', kw, src, 'module外部脚本', node);
                                    node.remove();
                                    break;
                                }
                            }
    ...
    const dataURIObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            mutation.addedNodes.forEach(function (node) {
                let src = getSafeSrc(node)
                    if (node.nodeName === 'SCRIPT' && src && src.startsWith('data:')) {
